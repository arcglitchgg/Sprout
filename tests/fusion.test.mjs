...
const { fuseFighters, getFusionEligibility } = load("@/lib/fusion");
...
test("rejects mixed species, mixed mutations, Golden, and Prismatic", () => {
  ...
  for (const mutation of ["golden", "prismatic"]) {
    assert.equal(fuseFighters(ids.map((id) => fighter(id, "potato", mutation)), ids, () => 0), null);
  }
});

test("Normal fusion uses the strict 0.70 boundary; Large stays Large", () => {
  ...
  let calls = 0;
  const large = fuseFighters(ids.map((id) => fighter(id, "carrot", "large")), ids, () => { calls++; return 0.2; });
  assert.equal(large.result.mutation, "large");
...
test("Save V2 round-trips the fused roster without a schema change", () => {
  ...
  const save = {
    ...
    game: {
      ...
      harvestedCrops: [], collection: [], fighters: fusion.remaining,
    },
