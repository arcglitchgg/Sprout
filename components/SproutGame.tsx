...
function SproutGameSession({ initialSave, scheduleSave }: { initialSave: SproutSaveV2 | null; scheduleSave: (payload: SproutSavePayloadV2) => void }) {
  ...
  const { coins, farmXp, farmLevel, unlockedPlotCount, selectedCrop, setSelectedCrop, seeds, buySeed, now, plots, collection, harvestedCrops, fighters, handlePlotClick, harvestAll, sellCrops, awakenCrop, fuseFighters, awardBattleVictory } = useGame(initialSave?.game, notify);
    ...
    <main className="h-dvh overflow-hidden bg-[#171c19] p-2 text-[#2f3e2f] sm:p-3">
      <div className="mx-auto flex h-full max-w-6xl flex-col">
        ...
        <div className="min-h-0 flex-1">
          <PixelWorld coins={coins} unlockedPlotCount={unlockedPlotCount} plots={plots} now={now} selectedCrop={selectedCrop} setSelectedCrop={setSelectedCrop} seeds={seeds} buySeed={buySeed} handlePlotClick={handlePlotClick} harvestAll={harvestAll} fighters={fighters} collection={collection} harvestedCrops={harvestedCrops} sellCrops={sellCrops} awakenCrop={awakenCrop} fuseFighters={fuseFighters} awardBattleVictory={awardBattleVictory} notifications={notifications} notify={notify} onDismissNotification={dismissNotification} initialFarmerTile={farmerWorld.farmerTile} initialFarmerFacing={farmerWorld.facing} onFarmerSettled={handleFarmerSettled} />
        </div>
