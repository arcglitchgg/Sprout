...
export default function Battle({ fighters, onVictory, onComplete }: { fighters: Fighter[]; onVictory: (result: BattleState) => void; onComplete?: (result: BattleState) => void }) {
  ...
  const team = selected.map((id) => fighters.find((fighter) => fighter.id === id)).filter((fighter): fighter is Fighter => Boolean(fighter));
  const valid = team.length === 3 && new Set(selected).size === 3;
  ...
  return (
    <section className="mt-6 rounded-2xl bg-[#f4e8c1] p-4">
      ...
      <TeamSelector fighters={fighters} selected={selected} onSelect={setSelected} locked={running || presenting} />
      ...
      <BattleArena key={battle?.id ?? "preview"} battle={battle} team={selected.map((id) => fighters.find((fighter) => fighter.id === id))} onProgress={updatePresentation} />
