...
export default function BattleFighter({ fighter, hp, slot, dialogue, impact, shield, moving, animationKey, animation }: {
  fighter: Fighter; hp: number; slot: number; dialogue?: string; impact?: string; shield: boolean; moving: boolean; animationKey: number | null; animation?: BattleAnimationName;
}) {
  return <div className="relative flex w-28 flex-col items-center text-center">
    ...
    <div key={impact ? animationKey : "rest"} className={`${impact ?? ""} ${hp === 0 ? "battle-ko" : ""} ${moving ? "opacity-0" : ""}`}>
